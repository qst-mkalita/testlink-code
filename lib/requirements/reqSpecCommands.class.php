...
class reqSpecCommands
{
  ...
  */
  function doCreate(&$argsObj, $request)
  {
    ...
    if ($ret['status_ok']) {
      ...
      logAuditEvent(
        TLS("audit_req_spec_created", $this->auditContext->tproject, $argsObj->title),
        "CREATE",
        $ret['id'],
        "req_specs"
      );
  ...
  */
  function doUpdate(&$argsObj, $request)
  {
    ...
    $chk = $this->reqSpecMgr->check_main_data(
      $argsObj->title,
      $argsObj->doc_id,
      $argsObj->tproject_id,
      $argsObj->parentID,
  ...
  */
  function doDelete(&$argsObj)
  {
    ...
    logAuditEvent(
      TLS("audit_req_spec_deleted", $this->auditContext->tproject, $req_spec['title']),
      "DELETE",
      $argsObj->req_spec_id,
      "req_specs"
    );
    ...
    if (!is_null($childNodes)) {
      ...
      for ($idx = 0; $idx < $loop_qty; $idx++) {
        ...
        if ($cNode['node_table'] == 'req_specs') {
          ...
          $this->commandMgr->doFreezeVersion($req_freeze_version, $request);
        }
      }
    }
